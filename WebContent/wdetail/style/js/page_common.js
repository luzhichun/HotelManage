...
$(function(){
	...
	function fun(currentPage){
		...
		$.ajax({
			url:"getFoodType",
			type:"POST",
			data:{"FTcurrentPage":currentPage},
			dataType:"json",
			success:function(json){
				...
				for(var i=0;i<result.length;i++){
					var typename=result[i].typename;
					var foodtypeid=result[i].foodtypeid;
					var l_li="<li><a href='menusByType?foodtypeid="+foodtypeid+"&&foodName="+null+"&&FFcurrentPage=1&&type=1&&FTcurrentPage="+currentPage+"'>"+typename+"</a></li>";
					var $li=$(l_li);
					$li.data("foodtypeid",foodtypeid);
					$("#typeList").append($li);
				}
			}
		});
